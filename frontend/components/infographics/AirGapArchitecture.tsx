  ...
  return (
    <div className="soc-panel p-5 space-y-3 font-mono select-none">
      <div className="flex items-center justify-between gap-2 border-b border-soc-border pb-2.5">
        ...
        <span className="text-[10px] uppercase tracking-[0.14em] text-soc-textDim">Sovereign Air-Gap</span>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
        {/* Local enclave: sealed boundary containing the local data path */}
        <div className="relative rounded-sm border border-dashed border-soc-borderStrong p-3 pt-4 lg:col-span-3">
          <span className="absolute -top-[6px] left-3 bg-soc-panel px-1.5 text-[9px] uppercase tracking-[0.14em] text-soc-textDim">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {localNodes.map((node) => {
              ...
              return (
                <div key={node.title} className="space-y-1.5 rounded-sm border border-soc-border bg-soc-overlay p-3">
                  <div className="flex items-center justify-between">
                    <Icon className="h-3.5 w-3.5 text-soc-textMuted" aria-hidden="true" />
                    <span className="rounded-sm border border-soc-ok/30 bg-soc-okDim px-1.5 py-px text-[9px] font-bold uppercase tracking-[0.08em] text-soc-ok">
                      {node.metric}
                    </span>
                  </div>
                  <div className="text-[11px] font-bold text-soc-text">{node.title}</div>
                  <p className="font-sans text-[11px] leading-snug text-soc-textSecondary">{node.desc}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="flex flex-col">
          <div className="hidden items-center gap-2 pb-2 lg:flex" aria-hidden="true">
            ...
            <span className="text-[9px] font-bold text-soc-crit">✕</span>
            <span className="h-px flex-1 bg-soc-border" />
          </div>
          {egressNode && (
            <div className="flex-1 space-y-1.5 rounded-sm border border-soc-crit/40 bg-soc-critDim/60 p-3">
              <div className="flex items-center justify-between">
                <Network className="h-3.5 w-3.5 text-soc-crit" aria-hidden="true" />
                <span className="rounded-sm border border-soc-crit/35 bg-soc-critDim px-1.5 py-px text-[9px] font-bold uppercase tracking-[0.08em] text-soc-crit">
                  {egressNode.metric}
                </span>
              </div>
              <div className="text-[11px] font-bold text-soc-text">{egressNode.title}</div>
              <p className="font-sans text-[11px] leading-snug text-soc-textSecondary">{egressNode.desc}</p>
            </div>
          )}
        </div>
      </div>
